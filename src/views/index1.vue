...
export default {
    ...
    mounted(){
        ...
        if(categories){
            // 如果当前是登录的状态，但是栏目的第一项居然不是“关注”，需要重新请求栏目列表
            // 如果当前未登录，但是栏目的第一项居然叫“关注”，也需要重新请求栏目列表
            if(!token && categories[0].name ==="关注" || token && categories[0].name !== "关注"){
                // 调用请求栏目的数据,并且保存到本地
                this.getCategories();
            }else{
    ...
    methods: {
        ...
        getList(){
            ...
            if(finished) return;
            //return直接跳出getLIst 这个函数 不会执行下面的axios
            const config = {
                url: "/post",
                params: {
        ...
        // scroll	滚动时触发，仅在 sticky 模式下生效	会返回这个对象{ scrollTop: 距离顶部位置, isFixed: 是否吸顶 }
        handelScroll(data){
